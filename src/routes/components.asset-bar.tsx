...
import { assetBarDemos } from "@/components/docs/demos/asset-bar-demos";
...
const doc = getDoc("components/asset-bar");
const toc = getTocFromDoc(doc);

export const Route = createFileRoute("/components/asset-bar")({
  head: () => ({
    meta: [
      { title: `${doc.frontmatter.title} - Scribe` },
      {
        ...
        content:
          ...
          "Scribe Asset Bar documentation for modal and takeover asset-selection actions.",
      },
    ],
  }),
  component: AssetBarDocsPage,
});

function AssetBarDocsPage() {
  const figmaUrl = getFigmaUrlForDocSlug("asset-bar");
  ...
  return (
    <DocsShell
      ...
      toc={toc}
    >
      <DocPageSections sections={doc.sections} demos={assetBarDemos} toc={doc.frontmatter.toc} />
    </DocsShell>
  );
}
