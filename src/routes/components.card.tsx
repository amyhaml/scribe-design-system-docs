...
import { cardDemos } from "@/components/docs/demos/card-demos";
...
const doc = getDoc("components/card");
    ...
    meta: [
      { title: `${doc.frontmatter.title} - Scribe` },
      {
        ...
        content:
          ...
          "Scribe Card documentation for listing and selection workflows.",
      },
    ],
  }),
  component: CardDocsPage,
});

function CardDocsPage() {
  const figmaUrl = getFigmaUrlForDocSlug("card");
  ...
  return (
    <DocsShell
      ...
      toc={toc}
    >
      <DocPageSections sections={doc.sections} demos={cardDemos} toc={doc.frontmatter.toc} />
    </DocsShell>
  );
}
