...
import { dropzoneDemos } from "@/components/docs/demos/dropzone-demos";
...
const doc = getDoc("components/dropzone");
const toc = getTocFromDoc(doc);

export const Route = createFileRoute("/components/dropzone")({
  head: () => ({
    meta: [
      { title: `${doc.frontmatter.title} - Scribe` },
      {
        ...
        content:
          ...
          "Scribe Dropzone documentation for upload and upload status states.",
      },
    ],
  }),
...
function DropzoneDocsPage() {
  const figmaUrl = getFigmaUrlForDocSlug("dropzone");
  ...
  return (
    <DocsShell
      ...
      toc={toc}
    >
      <DocPageSections sections={doc.sections} demos={dropzoneDemos} toc={doc.frontmatter.toc} />
    </DocsShell>
  );
}
