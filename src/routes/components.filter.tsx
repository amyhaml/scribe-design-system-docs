...
import { filterDemos } from "@/components/docs/demos/filter-demos";
...
const doc = getDoc("components/filter");
const toc = getTocFromDoc(doc);

export const Route = createFileRoute("/components/filter")({
  head: () => ({
    meta: [
      { title: `${doc.frontmatter.title} - Scribe` },
      {
        ...
        content:
          ...
          "Scribe Filter documentation for list filter rows and filter sets.",
      },
    ],
  }),
  component: FilterDocsPage,
});

function FilterDocsPage() {
  const figmaUrl = getFigmaUrlForDocSlug("filter");
  ...
  return (
    <DocsShell
      ...
      toc={toc}
    >
      <DocPageSections sections={doc.sections} demos={filterDemos} toc={doc.frontmatter.toc} />
    </DocsShell>
  );
}
