...
import { treeMenuDemos } from "@/components/docs/demos/tree-menu-demos";
...
const doc = getDoc("components/tree-menu");
const toc = getTocFromDoc(doc);

export const Route = createFileRoute("/components/tree-menu")({
  head: () => ({
    meta: [
      { title: `${doc.frontmatter.title} - Scribe` },
      {
        ...
        content:
          ...
          "Scribe Tree documentation for hierarchical lists and selectable tree menus.",
      },
    ],
  }),
  component: TreeMenuDocsPage,
});

function TreeMenuDocsPage() {
  const figmaUrl = getFigmaUrlForDocSlug("tree-menu");
  ...
  return (
    <DocsShell
      ...
      toc={toc}
    >
      <DocPageSections sections={doc.sections} demos={treeMenuDemos} toc={doc.frontmatter.toc} />
    </DocsShell>
  );
}
